  ...
  <el-card shadow="never" class="box-card" body-style="padding:0 16px" id="company-new-ipo-count-item"
    ...
    <div class="head-flex vux-1px-b">
      ...
      <div class="row r-pagination text">
        <router-link to="/kechuangban/kechuangbanTrend" target="_blank">
...
<script>
  export default {
    ...
    created() {
      this.getAuditStatusFromAuditing()
    },
    methods: {
      ...
      getAuditStatusFromAuditing(time) {
        ...
        this.$http({
          method: 'GET',
          url: this.$api.kechuangban.getStibAnalysisInfo,
          params: {}
        }).then((res) => {
          this.auditStatusFromAuditingLoading = false;
          if (res.data.returnCode == 200 || res.data.returnCode == 1) {
            this.auditStatusFromAuditingData = res.data.returnObject
            this.$nextTick(() => {
      ...
      echartComparison(id, data, text) {
         ...
         * 已发行=已发行 
         */
        var xData = ['已受理', '已问询', '上市委会议', '注册', '已发行'];
        var yData = data.map(e => {
          let num = e.number;
          let name = e.name
          if (e.name == '通过') {
            name = '上市委会议'
            ...
            formatter: function (data) {
              ...
              data.forEach((item) => {
                if (item.seriesName.indexOf('率') > -1) {
                  html += `<div style="display:flex;justify-content:space-between"><span>${item.axisValue}：</span><span>${item.value}%</span></div>`
                } else {
                  html += `<div style="display:flex;justify-content:space-between"><span>${item.axisValue}：</span><span>${item.value}</span></div>`
                }
              });
