<template>
  <div class="ipo-search" id="ipo-search" v-loading.fullscreen.lock="loading">
    ...
    <search @submitAll="submitAll" @screenIsActive="screenIsActive" @deleteType='deleteType' :changeSearch="true"
      ...
      <template slot="title">科创板IPO</template>
      <template slot="info">小安为您整理的科创板上会上市公司，和在审上市公司信息统计。每日及时更新，供您快速查阅。</template>
          ...
          <div class="box-container">
            ...
            <div :class="{noSort: this.$route.fullPath == '/ipo/search'}" v-else>
              ...
              <div class="listContent" style="padding: 0 16px">
                ...
                <v-list :data="this.listInfo" :urlType="'kechuangban'" @seniorButton="seniorMethod" :noInfo="isInfo"></v-list>
              </div>
...
<script>
  ...
  import VCompanyNewIPOCountItem from './components/companyNewIpoCountItem.vue'
...
//          {id: '1', name: '阅读量', isActive: false, up: true},
        ...
        // 筛选区
        screenListList: ['全部', '发布主体', '所属行业', '文件类别', '发布日期'], // 筛选默认显示的title格式
        screenList: [
          {id: 0, name: [], titleName: '全部', title: '全部', class: 'all', type: 'global', isSelect: true, list: []},
          {id: 1, name: [], titleName: '发布主体', title: '发布主体', class: '', type: 'market', isSelect: false, list: []},
    ...
    methods: {
      ...
      synchronizationRouter() {
        this.$router.push({
          path: '/kechuangban/kechuangbanIpo', query: {
      ...
      preparation() {
        ...
        this.$http({
          ...
          if (res.data.returnCode == 1 || res.data.returnCode == 200) {
            ...
            var market = content.SubTypeConfitions; // 板块类别
            var industry = content.secondIndustryConditions; // 所属行业
            var confitions = content.fileTypeConfitions; // 文件类别
            //
      ...
      handlerSearchList() {
        ...
        this.$http({
          ...
          params: {
            ...
            // marketTypeEnum: this.screenList[1].list.toString(), // 发布主体
            subTypeEnum: this.screenList[1].list.toString(), // 发布主体
            industryCodeLevelTwo: this.screenList[2].list.toString(), // 所属行业
            fileTypeEnum: this.screenList[3].list.toString(), // 文件类别
            // //
            pageNum: this.page.index, // 页数
            pageSize: this.page.size // 每页条数
          }
          ...
          if ((res.data.returnCode == 1 || res.data.returnCode == 200) && res.data.returnObject) {
            ...
            if (this.page.total == 0) {
              ...
              arrayListData.forEach((item, i) => {
                ...
                obj = {
                  ...
                  url: item.fileUrl,
                  date: this.$common.transTime(item.releaseDate)[0],
                  describe: htmlDesc,
