  ...
  <div id="groupset-yemanren">
    ...
    <el-card shadow="never" class="export-list">
      <div slot="header" class="header-box">
        <span>野蛮人管理
        <span class="warn-level" v-if="kongGuRatio > 0">
          本公司预警等级：低
          <i class="i1"></i><i :class="{'i2': kongGuRatio<=30}"></i><i :class="{'i3': kongGuRatio<=20}"></i><i :class="{'i4':  kongGuRatio<=10}"></i><i :class="{'i5': kongGuRatio<=5}"></i> 高</span>
        </span>
    ...
    <el-dialog class="dialogStockHolder" :visible.sync="setDialog" width="620px" top="35vh" center>
      <template slot="title">新建野蛮人分组</template>
    ...
    <el-dialog class="dialogStockHolder add-yemanren" :visible.sync="setDialog2" width="900px" top="10vh" center>
      <template slot="title">{{isAdd?'新建野蛮人分组':'添加野蛮人组成员'}}</template>
      <div class="">
        <div class="box-search">
          ...
          <el-input v-model="possessorName" placeholder="请输入内容" size="mini" class="possessor-name-ymr ml"></el-input>
    ...
    <el-dialog class="dialogStockHolder group-edit-dialog" :visible.sync="editDialog" width="900px" center>
      ...
      <div class="setkonggu-content">
        ...
        <el-table :data="editGroupItem.rollOfficialDataList" style="width: 100%" height="380" class="holderInfo" stripe size="mini">
          ...
          <el-table-column prop="possessorTypeName" label="股东性质" width="200" align="center">
          </el-table-column>
    ...
    <el-dialog class="dialogStockHolder setWarn" :visible.sync="warnDialog" width="620px" top="35vh" center>
      <template slot="title">野蛮人持股比例设置</template>
      <div>野蛮人持股比例合计变动值达到 <span class="text-primary">{{warnValue}}%</span> ，将在“最新股东分析”界面提醒。 </div>
      <div>
...
//            isOne: 1,
          ...
          if (res.data.returnCode == 200 || res.data.returnCode == 1) {
            ...
            // this.kongGuRatio = 3
            this.tableData = this.groupListInfo.YE_MAN_REN || [] // .rollOfficialDataList
