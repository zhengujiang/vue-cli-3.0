  ...
  <div id="groupset-yemanren">
    ...
    <el-card shadow="never" class="export-list">
      <div slot="header" class="header-box">
        <span>相同管理人旗下产品分组</span>
        <div class="header-form">
          <!--<el-select size="small" v-model="defaultRecordDate" placeholder="选择期数" class="recordDate-select" @change="defaultRecordDateChange">
            <el-option v-for="item in recordDate" :key="item" :label="item+'期'" :value="item">
            </el-option>
          </el-select>-->
          <div class="vxa-flax-box align vxa-border-data">
            <span class="row-span">当前期数</span>
            <el-select size="mini" v-model="defaultRecordDate" placeholder="选择期数" @change="defaultRecordDateChange">
              <el-option v-for="item in recordDate" :key="item" :label="item+'期'" :value="item">
              </el-option>
            </el-select>
          </div>
      ...
      <div class="listBox">
        <el-table :data="tableData" style="width: 100%" @sort-change="tableSortChange" :span-method="arraySpanMethod" stripe size="mini" >
          ...
          <el-table-column prop="groupPersonNumber" sortable="custom" label="成员数(个)" width="120" align="right">
          </el-table-column>
    ...
    <el-dialog class="dialogStockHolder" :visible.sync="setDialog" width="620px" top="35vh" center>
      <template slot="title">新建相同管理人旗下产品分组</template>
    ...
    <el-dialog class="dialogStockHolder add-yemanren" :visible.sync="setDialog2" width="900px" top="10vh" center>
      <template slot="title">{{isAdd?'新建相同管理人旗下产品分组':'添加相同管理人旗下产品分组成员'}}</template>
      <div class="">
    ...
    <el-dialog class="dialogStockHolder setWarn" :visible.sync="warnDialog" width="620px" top="35vh" center>
      <template slot="title">其他一致行动人持股比例设置</template>
      <div>其他一致行动人持股比例合计变动值达到 <span class="text-primary">{{warnValue}}%</span> ，将在“最新股东分析”界面提醒。 </div>
      <div>
...
//      possessorTypes: Array
    ...
    methods: {
      ...
      defaultRecordDateChange () {
        this.startRecordDate = this.defaultRecordDate
        this.endRecordDate = this.defaultRecordDate
        this.getGroupListInfo()
      },
      // 添加组操作
      // + 号操作
...
//        }
      ...
      getGroupListInfo () {
        this.$http({
          ...
          params: {
            userId: this.userId,
            groupType: 'MANAGER_FUND',
            recordDate: this.$common.getTimestampms(this.defaultRecordDate),
            isOne: 1,
          ...
          if (res.data.returnCode == 200 || res.data.returnCode == 1) {
            ...
            // this.kongGuRatio = 3
            this.tableData = this.groupListInfo.MANAGER_FUND || [] // .rollOfficialDataList
      ...
      addGroup () {
        ...
        this.$http({
          ...
          params: {
            ...
            recordDate: this.startRecordDate
          }
        }).then((res) => {
          if (res.data.returnCode == 200 || res.data.returnCode == 1) {
            this.setDialog = false
        ...
        this.$axios.gxfApi.searchHolderInfo(paras)
          .then((res) => {
            if (res.data.returnCode == 200 || res.data.returnCode == 1) {
              ...
              // this.recordDate = Object.assign([], this.recordDate)
            }
          })
      },
      // 获取名册期数
        ...
        this.$axios.gxfApi.getLastedRecordDate(paras)
          .then((res) => {
            if (res.data.returnCode == 200 || res.data.returnCode == 1) {
              ...
              this.defaultRecordDate = this.recordDate[0]
              this.getGroupListInfo()
            }
          })
      },
