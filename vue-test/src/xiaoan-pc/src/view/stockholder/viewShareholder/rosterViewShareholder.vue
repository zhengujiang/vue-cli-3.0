  ...
  <div id="rosterViewShareholder">
    <div class="header-box">
      <div class="header content">
        <span class="title">{{typeFun}}</span>
        <span class="time">（{{timeDateFun}}期）</span>
      </div>
    </div>
      ...
      <el-row :gutter="20">
        <el-col :span="18" v-loading="listLoading">
          <div class="check-container">
            <div class="col left">
              <el-input class="input-small" clearable size="small" @keyup.13.native="searchRollOfficial"
                        placeholder="搜索股东名称"
                        v-model="shareholderVal">
                <el-button slot="append" @click="searchRollOfficial" icon="el-icon-search"></el-button>
              </el-input>
            </div>
            <div class="col vxa-is-right">
              <span style="vertical-align: middle;color: #8D8D8D;font-size:12px">股东性质</span>
              <el-select @change="holderChange" size="mini" v-model="holderSelect" placeholder="请选择">
                <el-option v-for="item in holderOptions" :key="item.value" :label="item.label" :value="item.value">
                </el-option>
              </el-select>
          ...
          <div style="padding-top: 16px">
            <el-table @sort-change="sortChange" @header-click="headerCellChange" min-height="600" class="table-box-form"
                      :data="tableData" style="width: 100%;min-height: 600px;" stripe>
              <!---->
              <el-table-column header-align="center" :sortable="false" prop="index" show-overflow-tooltip
                               label="序号" align="center" width="50px">
                <div slot-scope="scope" class="set-index">{{((page.index * page.size) - (page.size-1)) + scope.$index}}</div>
              </el-table-column>
              <el-table-column class-name="investor-name" header-align="center" :sortable="false" prop="possessorName"
                               label="股东名称" align="center" width="151px">
              ...
              <el-table-column header-align="center" :sortable="false" prop="possessorType" show-overflow-tooltip
                               label="股东性质" align="center" width="135px">
              ...
              <el-table-column header-align="right" sortable="custom" prop="possessorAmount" show-overflow-tooltip
                               label="持股数量(股)" align="right" width="110px">
                <div slot-scope="scope" class="set-padding">
                  {{amountAbs(scope.row.possessorAmount)}}
                </div>
              </el-table-column>
              <el-table-column header-align="right" sortable="custom" prop="possessorRatio" show-overflow-tooltip
              ...
              <el-table-column header-align="right" sortable="custom" prop="shareRestrictAmount" show-overflow-tooltip
                               label="限售股(股)" align="right" width="120px">
                <div slot-scope="scope" class="set-padding">
              ...
              <el-table-column header-align="right" sortable="custom" prop="pledgeFreezeAmount" show-overflow-tooltip
                               label="质押/冻结(股)" align="right" width="120px">
                <div slot-scope="scope" class="set-padding">
                  {{amountAbs(scope.row.pledgeFreezeAmount)}}
                </div>
              </el-table-column>
              <!--<el-table-column header-align="center" prop="messageAddress" show-overflow-tooltip label="地址"
...
<script>
  ...
  export default {
     ...
     * PUBLIC_FUND(1, "公募基金"),
   PRIVATE_FUND(2, "私募基金"),
   ASSET_MANAGEMENT_OF_SC(3, "证券公司资管产品"),
   HK_CLEARING_CORP(4, "香港结算公司"),
   Q_F_I_I(5, "QFII"),
   R_Q_F_I_I(6, "RQFII"),
   HUI_JINg_COMPANY(7, "汇金公司"),
   CP(8, "中证金"),
   SOCIAL_SECURITY_FUND(9, "社保基金"),
   OTHER(10, "一般机构或产品"),
   NATURAL_PERSON(11, "自然人"),
   */
        natureOption: {
          '1': '公募基金',
          '2': '私募基金',
    ...
    computed: {
      ...
      typeFun() {
        ...
        console.log(typeof this.type, title);
        return title;
      }
    },
    methods: {
      ...
      addExportRollOfficial(type) {
        ...
        this.checkedNum = this.nextCheckNum;
        switch (type) {
          case 1:
            this.exportRollOfficialDigTitle = '导出显示信息'
            break;
          case 2:
            // this.checkBoxNumOption.forEach((item) => {
            //   item.disabled = false;
            // })
      ...
      exportRollOfficial() {
        let params = {
          ...
          recordDate: this.timeDateFun,
          count: this.radioOption[this.checkedNum],
          rollType: this.type,
          exportType: this.exportType
        };
        this.$axios.gxfApi.exportRollOfficial(params).then(res => {
      ...
      searchRollOfficial() {
        ...
        let params = {
          ...
          rollType: this.type,
          ratioOrderType: this.sortTable.possessorRatio,
        ...
        this.$axios.gxfApi.searchRollOfficial(params).then(res => {
          ...
          if (res.data.returnCode == 1 || res.data.returnCode == 200) {
            ...
            if (!this.firstTime) {
              ...
              this.nextCheckNum = this.checkedNum;
              this.firstTime = true;
            }
