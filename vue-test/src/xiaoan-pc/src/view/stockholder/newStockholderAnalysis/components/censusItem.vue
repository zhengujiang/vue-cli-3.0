        ...
        this.$axios.gxfApi.getShareholderStatistics(params).then(res => {
          if (res.data.returnCode == 200) {
            ...
            if (dataObject.circulateAllAmount == null) {
              ...
              this.circulateAllAmountDisplay = true
            }
            let dataP = [
              {
                name: '个人(股)',
        ...
        let option = {
          ...
          tooltip: {
            ...
            formatter: function (data) {
              let html = ``;
              html += `<div style="display:flex;color:#8d8d8d;justify-content:space-between"><span>${data.name}：</span><span style="color: #AEAEAE">${that.amountAbs(data.value)}</span></div>`
              return html
            }
          },
          color: color,
