  ...
  <ul class="list">
    <li v-for="(item, index) of listData" :key="index" class="vux-1px-b hover">
      <div class="title-default">
        <p class="row title effect" :class="{'flex1': item.show}" v-if="item.timeliness == '失效'">
          <router-link class="a-link" :class="{fontWeight: item.show}" :to="`/rule/details?id=${item.id}`"
                       target="_blank"
                       v-html="item.title"></router-link>
          <i v-if="item.newLogo" class="newLogo">new</i>
        </p>
        <p class="row title" :class="{'flex1': item.show}" v-else>
          <router-link class="a-link" :class="{fontWeight: item.show}" :to="`/rule/details?id=${item.id}`"
                       target="_blank"
                       v-html="item.title"></router-link>
          <i v-if="item.newLogo" class="newLogo">new</i>
      ...
      <el-collapse-transition>
        <div class="desc-all" v-if="item.show">
          <div class="desc" v-if="item.describe">
            <router-link class="a-link desc-link hiddenLine4" :to="`/rule/details?id=${item.id}`" target="_blank"
          ...
          <div class="about">
            <div class="row left">
              ...
              <span class="form" v-if="item.plate.length>0">&ensp;|&ensp;</span>
              <span>
