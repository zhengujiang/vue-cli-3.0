<template>
  <div id="change-company">
    <header class="clearFix">
      <div class="card-title" style="display: inline-block;float:left"><span>当前套餐：{{userInfo.enterpriseCompanyName}} </span><span v-if="userInfo.root" class="role">管理员</span></div>
        ...
        <el-col :span="12" v-for="(item, index) in companyList" :key="index">
          <div class="enterprise-box" :class="{'is-active':userInfo.companyId == item.id}">
            ...
            <p class="text" style="height: 20px">
              <span v-if="item.authorityRole == 'ENTERPRISE'">
                成员数量 ：{{item.userCount}}/{{item.userLimit}}人
                <span v-if="userInfo.id == item.rootUserId">，剩余
                  <span class="highlight">{{item.inviteCount}}</span>
                  人可邀请
                </span>
              </span>
            </p>
            <p class="text" style="height: 20px">
              <span v-if="item.authorityRole == 'ENTERPRISE'">
                有效期至{{$common.transTime(item.endDate)[0]}} ，剩余<span class="highlight">{{remainTime(item)}}</span>天
              </span>
            </p>
            <div class="btn-group">
    ...
    <el-dialog class="dialog-class" title="续费" :visible.sync="dialogVisible" width="620px">
      <div class="renew-company">
        ...
        <p>方案名称：&nbsp;企业版 </p>
        <p>方案时长：&nbsp;年付 </p>
        <p>方案价格：&nbsp;<span class="price">￥9999&nbsp;</span><span class="rmb">￥</span><span class="money">29999</span></p>
        <p>到期时间：&nbsp;{{newdate}}</p>
      </div>
      <div class="bank-account">
        <p class="order">点击提交订单后，客服人员会在1-2个工作日内联系您注册的手机号码。</p>
        <p class="remit bank">您还可以通过银行汇款的方式进行充值，以下是具体信息：</p>
        <p class="remit">账户名称：&nbsp;上海信公印诚信息技术有限公司</p>
        <p class="remit">开户银行：&nbsp;中国银行上海市延安东路支行</p>
        <p class="remit">银行账户：&nbsp;457268920707</p>
  ...
  methods: {
    ...
    renew(item) {
      ...
      if (t1 > t) {
        ...
        let n = new Date(this.nowTime);
        let year = n.getFullYear() + 1;
        ...
        this.$http({
          ...
          .then(res => {
            ...
            if (res.data.returnCode == 200 || res.data.returnCode == 1) {
              // 当前企业默认排第一
              this.nowTime = res.data.returnObject.nowTime
              if (res.data.returnObject.list.length !== 0) {
                this.companyList = res.data.returnObject.list.sort(
                  (x, y) =>
    ...
    remainTime(item) {
      let time = -moment(this.$common.transTime(this.nowTime)[1]).diff(
