          ...
          <div class="item" v-for="(list,index) in shareContent" @click="linkContent(list.id)">
            <el-row>
              ...
              <el-col :span="3" v-if="list.share == true">
                <i class="iconfont web-bianji iconimg cancel" v-if="list.isDefault===0" @click.stop="linkEdit(list.id)" style="color:#ffb148;font-size: 17px;margin-top: -1px;"></i>
              </el-col>
              <el-col :span="6" v-if="list.share == false">
                <i class="iconfont web-bianji iconimg cancel" v-if="list.isDefault===0" @click.stop="linkEdit(list.id)" style="color:#ffb148;float: right;font-size: 17px;margin-top: -1px;"></i>
              </el-col>
              <el-col :span="3" v-show="list.share">
                <i class="iconfont web-fenxiang2 iconimg cancel" v-if="list.isDefault===0" @click.stop="cancelShare(list.id, list.favouriteName)" style="color:#ffb148;"></i>
              </el-col>
              <el-col :span="3">
                <i class="iconfont web-shanchu iconimg cancel" style="color:#ffb148" v-if="list.isDefault===0" @click.stop="delCollect(list.id, list.favouriteName)"></i>
              </el-col>
            </el-row>
            ...
            <el-row class="fbc2 iconRow">
              ...
              <el-col :span="6">
                <i class="iconfont web-gonggao2" style="color:#ffd091;font-size: 17px;"></i> &nbsp;&nbsp;{{ list.caseCount }}
              </el-col>
  ...
  methods: {
    ...
    newFolder() {
      ...
      if (this.shareContent.length >= 10) {
        ...
        this.$router.push({ path: '/collect/addCollect' })
      }
    },
    linkContent(index) {
      this.$router.push({ path: '/collect/collectDetails?id=' + index })
      // this.$router.push({path: '/collect/equityIncentive?id=' + index})
      // this.$message({showClose: true, type: 'error', duration: 3000, message: index})
    },
    linkEdit(id) {
    ...
    getMyDefaultShare() {
      this.$http({
        ...
        if (res.data.returnCode == 1) {
          ...
          var defaultArr = [{
            id: content.id,
            createTime: this.$common.transTime(content.createTime)[1],
            favouriteName: content.favouriteName,
            caselawCount: content.caselawCount,
            caseCount: content.caseCount,
            qaCount: content.qaCount,
            share: false,
            isDefault: 1,
            lawCount: content.lawCount,
      ...
      this.$http({
        ...
        if (res.data.returnCode == 1) {
          if (res.data.returnObject) {
            // this.shareContent.concat(res.data.returnObject.list)
            let list = res.data.returnObject.list
            for (let i in list) {
              list[i].createTime = this.$common.transTime(list[i].createTime)[1]
              this.shareContent.push(list[i])
            }
            // console.log('collectContent', this.shareContent)
