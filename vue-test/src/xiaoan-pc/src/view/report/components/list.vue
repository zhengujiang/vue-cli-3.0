    ...
    <li v-for="(item, index) of listData" :key="index" class="vux-1px-b hover">
      <div class="title-default">
        <p class="row title" :class="{flex1: item.show}">
          <router-link class="a-link" :class="{fontWeight: item.show}" :to="`/report/details?id=${item.id}`" target="_blank"
        ...
        <div class="row right" v-if="!item.show">
          ...
          <span class="time">{{item.issued}}&ensp;|&ensp;</span>
          <span class="set">
      ...
      <el-collapse-transition>
        <div class="desc-all" v-if="item.show">
          <div class="desc" v-if="item.describe">
            <router-link class="a-link desc-link hiddenLine2" :to="`/report/details?id=${item.id}`" target="_blank"
                         v-html="item.describe"></router-link>
          </div>
          <div class="about">
            <div class="row left">
              <span class="form" v-if="item.date">{{item.date}}</span>
              <span class="allow" v-if="item.timeliness">|&ensp;{{item.timeliness}}</span>
              <span class="allow" v-if="item.biaoqian">|&ensp;{{item.biaoqian}}</span>
              <span class="time">|&ensp;{{item.issued}}</span>
            </div>
            <div class="row right">
            <span class="set">
